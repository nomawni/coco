...
HTTPServer::HTTPServer(std::shared_ptr<boost::asio::ip::tcp::socket> sock):
	m_sock(sock),
	m_request(4096),
...
void HTTPServer::start_handling() {
...
void HTTPServer::on_request_line_received(
	const boost::system::error_code& ec,
	std::size_t bytes_transferred)
{
	if (ec != 0) {
...
void HTTPServer::on_headers_received(const boost::system::error_code& ec,
	std::size_t bytes_transferred)
{
	if (ec != 0) {
		std::cout << "Error occured! Error code = "
...
void HTTPServer::send_response() {
	...
	response_buffers.push_back(
		boost::asio::buffer(m_response_status_line));
	...
	if (m_resource_size_bytes > 0) {
		response_buffers.push_back(
			boost::asio::buffer(m_resource_buffer.get(),
				m_resource_size_bytes));
	}
...
void HTTPServer::on_response_sent(const boost::system::error_code& ec,
	std::size_t bytes_transferred)
{
	if (ec != 0) {
		std::cout << "Error occured! Error code = "
